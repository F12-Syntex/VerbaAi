...
class MenuManager {
    constructor(apiKey, configManager, onActionHandler, onConfigHandler) {
        this.apiKey = apiKey;
        this.configManager = configManager;
        this.onActionHandler = onActionHandler;
        this.onConfigHandler = onConfigHandler;
    ...
    _createMenu(buttonContainer) {
        ...
        const menuWidth = 200; // Increased width for custom prompts
    ...
    _getMenuOptions() {
        ...
        if (Object.keys(customPrompts).length > 0) {
            baseOptions.push(null); // separator
            
            Object.entries(customPrompts).forEach(([id, promptData]) => {
                baseOptions.push({
                    icon: `<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                    ...
                    action: () => this.onActionHandler(id),
                    disabled: !this.apiKey
                });
            });
        }
